...
{  
    ...
    cmd='df -T | awk '\''NR>1 && ($2=="xfs" || $2=="ext4") && $1~/^\/dev\/sd[a-z][0-9]+$/ {print $1, $2}'\'' | while read fs type; do echo "Filesystem: $fs, Type: $type"; sg_modes -6 "$fs"; done'
    capture_system_info_command "Inspecting FUA support functionality as claimed by Disk, sg_modes" "$cmd"
