...
if [ ! -e "/etc/krb5.conf" ]; then
    ...
    if [ -e "/etc/krb5.conf" ]; then
        DEFAULT_LOG=$(get_conf_option '/etc/krb5.conf' 'logging' 'default' 'FILE:/var/log/krb5/krb5kdc.log') | cut -d ":" -f2 | cut -d "=" -f2
        KDC_LOG=$(get_conf_option '/etc/krb5.conf' 'logging' 'kdc' 'FILE:/var/log/krb5/krb5kdc.log') | cut -d ":" -f2 | cut -d "=" -f2
        ADMIN_SERVER_LOG=$(get_conf_option '/etc/krb5.conf' 'logging' 'admin_server' 'FILE:/var/log/krb5/krb5kdc.log') | cut -d ":" -f2 | cut -d "=" -f2
    fi
